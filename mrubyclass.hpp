...
class Class : public Module
{
	...
	static mrb_value constructor(mrb_state* mrb, mrb_value self)
	{
		START_HANDLE_ERROR
		{
			...
			{
				HANDLE_ERROR_AND_EXIT error_argument_count(mrb, str, "initialize", argc, sizeof...(TConstructorArgs));
			}
	...
	static mrb_value default_constructor(mrb_state* mrb, mrb_value self)
	{
		START_HANDLE_ERROR
		{
			...
			if (argc != 0)
			{
				HANDLE_ERROR_AND_EXIT error_argument_count(mrb, str, "initialize", argc, 0);
			}
...
public:
	...
	static mrb_value mruby_member_setter(mrb_state* mrb, mrb_value self)
	{
		try
		{
			...
			if (argc != 1)
			{
				std::string class_name = TypeBinder<std::string>::from_mrb_value(mrb, self);
				throw error_argument_count(mrb, class_name, name, argc, 1);
			}
